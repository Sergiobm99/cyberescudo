...
<main class="content-page">
  ...
  <div class="card">
    <div style="margin-bottom: 1.5rem; border-bottom: 1px solid var(--border); padding-bottom: 1rem;">
      <h2 style="font-size: 1.4rem; color: var(--white); margin-bottom: 0.5rem;"><?= $lang==='es' ? 'Generador de Hashes' : 'Hash Generator' ?></h2>
      <p style="color: var(--gray); font-size: 0.9rem;"><?= $lang==='es' ? 'Calcula hashes SHA de forma instantánea y segura. Ningún dato se envía al servidor.' : 'Calculate SHA hashes instantly and securely. No data is sent to the server.' ?></p>
    </div>

    <div class="cyber-input-wrapper" style="align-items: flex-start;">
      <textarea id="hash-input" class="cyber-input" style="min-height:120px; resize:vertical; padding-right:1.25rem;" placeholder="<?= $lang==='es' ? 'Escribe o pega el texto aquí...' : 'Type or paste text here...' ?>"></textarea>
    </div>

    <div id="hash-results" style="margin-top: 1.5rem;">
        <div style="color:var(--gray); text-align:center; padding: 1rem; font-family:var(--mono); font-size:0.9rem;">
            <?= $lang==='es' ? 'Empieza a escribir para generar los hashes.' : 'Start typing to generate hashes.' ?>
        </div>
    </div>
  </div>
</main>
